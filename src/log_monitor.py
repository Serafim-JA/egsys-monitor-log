    ...
    while True:
        ...
        if len(path_chosen) == 3:
            ...
            if not host or not user or not password:
                console.print(
                    f"[bold red]Erro: Variáveis de ambiente HOST_{client.upper()}{server.upper()}, USER{client.upper()}{server.upper()} ou PASSWORD{client.upper()}_{server.upper()} não encontradas no .env.[/bold red]")
                console.input("[yellow]Pressione Enter para continuar...[/yellow]")
                continue
